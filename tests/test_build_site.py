...
class BuildSiteTests(unittest.TestCase):
    ...
    def test_build_site_writes_index_chapters_and_nojekyll(self):
        ...
        self.write_chapter(root / "chapters" / "05-beaches.md", "Stranden", 6, "Beach body.", status="draft")
        self.write_chapter(root / "chapters" / "08-restaurants.md", "Restaurants", 9, "Restaurant body.", status="draft")
        self.write_chapter(root / "translations" / "en" / "chapters" / "00-start.md", "Start", 1, "Welcome.", status="draft")
        self.write_chapter(root / "translations" / "en" / "chapters" / "05-beaches.md", "Beaches", 6, "Beach body.", status="draft")
        ...
        beaches_html = (output_dir / "chapters" / "05-beaches.html").read_text(encoding="utf-8")
        self.assertIn("Topkeuzes in dit hoofdstuk", beaches_html)
        self.assertIn('data-ranking-table="05-beaches"', beaches_html)
        self.assertIn("<option value=\"rating\">Rating hoog-laag</option>", beaches_html)
        self.assertLess(beaches_html.index("Plage de Pampelonne"), beaches_html.index("Plage de la Nartelle"))
        self.assertIn("★★★★★", beaches_html)
        self.assertIn("€€€€", beaches_html)
        self.assertIn("Guide review", beaches_html)
        restaurants_html = (output_dir / "chapters" / "08-restaurants.html").read_text(encoding="utf-8")
        self.assertLess(restaurants_html.index("La Vague d&#x27;Or"), restaurants_html.index("La Tarte Tropézienne"))
        self.assertIn("sortRankingTable", restaurants_html)
        english_index = (output_dir / "en" / "index.html").read_text(encoding="utf-8")
        english_beaches = (output_dir / "en" / "chapters" / "05-beaches.html").read_text(encoding="utf-8")
        ...
        self.assertIn("Top choices in this chapter", english_beaches)
